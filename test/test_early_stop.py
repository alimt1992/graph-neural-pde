...
class EarlyStopTests(unittest.TestCase):
  def setUp(self):
    ...
    opt = {'dataset': 'Cora', 'self_loop_weight': 1, 'leaky_relu_slope': 0.2, 'beta_dim': 'vc', 'heads': 2, 'K': 10,
           ...
           'earlystopxT': 3, 'max_iters': 10, 'beltrami': False, 'batch_size': 1, 'multi_modal': False}
    self.opt = {**OPT, **opt}
  ...
  def test_block(self):
    ...
    t = 1
    out_dim = 6
    func = LaplacianODEFunc(self.dataset.data.num_features, out_dim, self.opt, self.device)
