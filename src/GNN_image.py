...
class GNN_image(BaseGNN):
  def __init__(self, opt, num_classes, num_features, device=torch.device('cpu')):
    ...
    self.odeblocks = nn.ModuleList(
      [self.block(self.f, self.regularization_fns, opt, device, t=time_tensor) for dummy_i in
       range(self.n_ode_blocks)]).to(self.device)
    self.odeblock = self.block(self.f, self.regularization_fns, opt, device, t=time_tensor).to(self.device)
