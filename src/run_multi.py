...
from data_multi import grid_edge_index_calc, get_dataset
from utils import to_dense_adj, get_rw_adj
from post_analysis import print_image_T, print_image_path, plot_att_heat, plot_att_edges
...
def train(epoch, model, optimizer, dataset):
  ...
  for batch_idx, batch in enumerate(loader):
    ...
    loss = lf(out, batch['labels'].view(-1))  #squeeze now needed
...
def main(opt):
  ...
  try:
    if opt['use_image_defaults']:
      opt = get_multimodal_opt(opt) #get_cora_opt(opt)
  except KeyError:
  ...
  model = GNN_multimodal(opt, opt['num_class'], opt['num_features'], device).to(device)
  ...
  for epoch in range(opt['epoch']): #1, opt['epoch']):
    ...
    train_acc, loss = train(epoch, model, optimizer, train_dataset)
  ...
  # print('best val accuracy {:03f} with test accuracy {:03f} at epoch {:d}'.format(best_val_acc, test_acc, best_epoch))
    ...
    log = 'Epoch: {:03d}, Runtime {:03f}, Loss {:03f}, forward nfe {:d}, backward nfe {:d}, Validation: {:.4f}'
    print(log.format(epoch, time.time() - start_time, loss, model.fm.sum, model.bm.sum, val_acc))
